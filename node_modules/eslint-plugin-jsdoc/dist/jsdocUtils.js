...
const tagNames = _tagNames.closureTags;
...
const tagsWithMandatoryType = [// These both show curly brackets in the doc signature and examples
// "typeExpression"
'implements', // "typeName"
'type']; // All of these have a signature with "type" except for
//  `augments`/`extends` ("namepath")
//  `param`/`arg`/`argument` (no signature)
//  `property`/`prop` (no signature)
// `modifies` (undocumented)

const tagsWithOptionalType = [// These have the example showing curly brackets but not in their doc signature, e.g.: https://jsdoc.app/tags-enum.html
...
'returns', 'return', 'throws', 'exception', 'yields', 'yield', // Todo: Omit these GCC specific items when in non-GCC mode after landing https://github.com/gajus/eslint-plugin-jsdoc/issues/356
// Shows the signature with curly brackets but not in the example
// "typeExpression"
'package', 'private', 'protected', // These do not show a signature nor show curly brackets in the example
'public', 'static', // Has no documentation, but test example has curly brackets, and
//  "name" would be suggested rather than "namepath" based on example; not
//  sure if name is required
'modifies']; // None of these show as having curly brackets for their name/namepath
...
const tagsWithOptionalNamepath = [...namepathDefiningTags, // `borrows` has a different format, however, so needs special parsing;
...
const tagsWithMandatoryNamepath = [// "name" (and a special syntax for the `external` name)
'external', 'host', // "namepath"
'callback', 'name', 'typedef'];
const tagsWithMandatoryTypeOrNamepath = [// "namepath"
